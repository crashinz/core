...
require_once __DIR__.'/pool_turn_clock.php';
...
function eight_ball_validate_settings(array $s,string $mode,array $definition=[]): array {
    if(array_diff(array_keys($s),['tableMode','pocketCalls','variant','botSeat2Difficulty','turnSeconds']))eight_ball_fail('Unsupported pool option.','SETTINGS_INVALID');
    $seconds=filter_var($s['turnSeconds']??30,FILTER_VALIDATE_INT);if(!in_array($seconds,[30,45,60],true))eight_ball_fail('Choose 30, 45 or 60 seconds.','SETTINGS_INVALID');
...
function eight_ball_settings_projection(array $s,string $mode,array $definition=[]): array {
    $s=eight_ball_validate_settings($s,$mode);return ['label'=>'Game Options','description'=>'Play a two-player match, or practice on your own.','controls'=>[
        ...
        ['key'=>'turnSeconds','type'=>'button-choice','label'=>'Time per shot','description'=>'Matches and bot games only. The avatar outline counts down; time pauses while balls move. Solo practice is untimed.','value'=>$s['turnSeconds'],'defaultValue'=>30,'options'=>[['value'=>30,'label'=>'30 seconds'],['value'=>45,'label'=>'45 seconds'],['value'=>60,'label'=>'60 seconds']]],
...
function eight_ball_rules_projection(array $s,string $mode,array $definition=[]): array {
    ...
    return ['label'=>'8 Ball rules','description'=>'Digital 8 Ball with selectable pocket-calling rules. Solo free shooting has no opponent or foul penalties.','sections'=>[
        ...
        ['label'=>'Shot clock','text'=>'Choose 30 (default), 45 or 60 seconds per shot. Placement and pocket calls use the same time. The clock waits for balls to stop. Running out is a foul: the opponent gets ball in hand, behind the head string if the break has not been played. Solo practice is untimed.'],
        ...
        ['label'=>'Break choices','text'=>'An illegal break lets the incoming player accept the table or request a new rack with either player breaking. If the 8 goes in on a legal break, the player who broke chooses to put it back and continue, or rerack all the balls and take the first shot again. If the cue ball also goes in, the opponent chooses to put the 8 back and place the cue ball behind the starting line, or rerack and take the first shot.'],
...
function eight_ball_initial_state(array $players,array $c=[]): array {
    ...
    if(isset($players[1]))$cues[$players[1]]=3; // Royal sapphire distinguishes Player 2.
    if(!empty($c['roundContext']['rematchContinues']))foreach($players as$id){
        $cue=filter_var($c['roundContext']['previousState']['cues'][$id]??$cues[$id],FILTER_VALIDATE_INT);
...
function eight_ball_resolve_shot(array &$s,int $actor,array $payload,array $result): void {
    ...
    if($before['breakShot']){
        ...
        if(isset($pots[8])){$s['phase']='choice';$s['turnIndex']=$scratch?$other:$before['turnIndex'];$s['choice']=$scratch?'eight-scratch':'eight-break';$s['statusText']=$scratch?'8 and cue ball pocketed on break. Choose how to continue on the table.':'8 pocketed on break. Choose how to continue on the table.';return;}
