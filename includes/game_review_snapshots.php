...
function game_review_snapshot_file(string $key): array
{
    ...
    if(!is_file($path)||!hash_equals($entry['sha256'],hash_file('sha256',$path)))throw new MultiplayerGameException('The frozen reference is missing or changed. Restore its private backup.','GAME_REVIEW_REFERENCE_INTEGRITY',409);
...
function game_review_snapshot_projection(array $review): array
{
    ...
    $frame['publicId']=$review['id'];$frame['stateVersion']=$cursor+1;$frame['nowUnixMs']=(int)floor(microtime(true)*1000);
...
function game_review_snapshot_dispatch(array $user,array $source): array
{
    ...
    if($action==='extension-action'){
        ...
        if((int)($source['expected_version']??-1)!==$cursor+1)throw new MultiplayerGameException('The reference changed. Try again.','MULTIPLAYER_GAME_STATE_CONFLICT',409);
