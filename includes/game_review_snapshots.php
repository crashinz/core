...
{
    static $manifest;
    return $manifest ??= json_decode((string)file_get_contents(__DIR__.'/game_review_snapshot_v4.json'),true,512,JSON_THROW_ON_ERROR);
}
...
function game_review_snapshot_file(string $key): array
{
    ...
    $root=security_private_storage_directory('game-review-snapshots').'/v4';$path=$root.'/'.$key;
