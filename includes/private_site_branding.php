...
function private_site_branding_inline_markdown(string $text): string {
    ...
    $escaped = preg_replace_callback(
        ...
        static function (array $match): string {
            ...
            $allowedLocal = ['README.md', 'AUTHORS.md', 'games/vendor/marcher/corechat.patch', 'games/vendor/marcher/BUILD.md', 'games/vendor/gnubg/corechat_api.c', 'games/vendor/gnubg/BUILD.md', 'games/vendor/gnubg/build.py'];
...
function private_site_branding_render_modifications(string $markdown): string {
    ...
    foreach (preg_split('/\R/', $markdown) ?: [] as $line) {
        ...
        if (preg_match('/^(#{1,3})\s+(.+)$/', $trimmed, $match)) {
            ...
            $anchor = match ($match[2]) { 'Stockfish' => ' id="stockfish"', 'Marcher' => ' id="marcher"', 'GNU Backgammon' => ' id="gnubg"', default => '' };
