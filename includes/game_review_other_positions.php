...
function game_review_other_position(array $s,array $case): array
{
    ...
    if($game==='hearts') {
        ...
        if($mode==='deal')$steps=[game_review_step(2,'deal')];
            ...
            $s['unoDeclared']['1']=$mode==='finish';$steps=[game_review_step(1,$mode==='draw'?'draw':'play',$mode==='draw'?[]:['card'=>$card,'color'=>'G'])];
        ...
        if($mode==='deal'){$steps=[game_review_step(1,'bet',['amount'=>10]),game_review_step(2,'bet',['amount'=>10]),game_review_step(1,'deal')];}
            ...
            $steps=[game_review_step(1,$mode==='finish'?'stand':$mode)];if($mode==='finish')$steps[]=game_review_step(1,'dealer-play');$expected='The prepared player choice deals or settles the correct hands and chips, without writing normal records.';
        }
        ...
        if($mode==='hit'){$s['levelElapsedMs']=0;$s['orbs']=[['x'=>100.0,'y'=>150.0]];}
        if($mode==='finish')$s['levelElapsedMs']=35000;
        $steps=[game_review_step(1,'arcade-input',['sequence'=>1,'left'=>false,'right'=>true,'fire'=>true])];
