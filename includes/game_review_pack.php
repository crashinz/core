...
{
    static $value;
    return $value ??= json_decode((string)file_get_contents(__DIR__.'/game_review_pack_v4.json'),true,512,JSON_THROW_ON_ERROR);
}
...
function game_review_pack_path(string $key): string
{
    ...
    foreach(explode('/','v4/'.$key) as $part){$path.='/'.$part;if(is_link($path))throw new RuntimeException('A reference path is linked. Restore it as a regular file.');}
    return $path;
}
...
function game_review_pack_status(): array
{
    ...
    return ['version'=>'v4','base'=>$base,'classic'=>$media];
}
...
function game_review_pack_install(string $archive): array
{
    ...
    if(!game_review_pack_matches($archive,$d))throw new RuntimeException('This ZIP does not match reference pack v4. Download the linked ZIP and try again.');
    return game_review_pack_locked(static function()use($archive){
        ...
        try{
            ...
            // Keep v3 intact; reuse only independently verified local Classic reference media.
            $classicCopied=0;$oldRoot=security_private_storage_directory('game-review-snapshots').'/v3';
            foreach(game_review_snapshot_manifest()['files'] as $key=>$entry){
                ...
                if($linked||!game_review_pack_matches($old,$entry))continue;
                $stream=fopen($old,'rb');try{$classicCopied+=(int)game_review_pack_copy_stream($stream,$key,$entry);}finally{fclose($stream);}
            }
            return ['added'=>$added,'reused'=>count($expected)-$added,'classicCopied'=>$classicCopied];
        }finally{$zip->close();}
    });
}
...
function game_review_pack_upload(array $actor,string $action,array $input,array $upload=[]): array
{
    ...
    try{
        ...
        if($action==='begin'){
            if(($input['size']??0)!==game_review_pack_descriptor()['bytes'])throw new RuntimeException('Choose the linked reference pack v4 ZIP.');
