...
function space_arcade_coop_action(array $state,int $actor,string $action,array $payload,array $context): array
{
    ...
    if($action==='arcade-input') {
        if(array_diff(array_keys($payload),['sequence','frameStart','frames']) || !is_int($payload['sequence'] ?? null) || $payload['sequence']<1 || !is_int($payload['frameStart'] ?? null) || $payload['frameStart']<0 || $payload['frameStart']%ARCADE_STEP_MS!==0 || !is_array($payload['frames'] ?? null) || !array_is_list($payload['frames']) || count($payload['frames'])<1 || count($payload['frames'])>50) throw new MultiplayerGameException('Invalid input frames.','ARCADE_INPUT_INVALID',422);
        $ticks=0;
        foreach($payload['frames'] as $run) {
            if(!is_array($run) || array_diff(array_keys($run),['ticks','left','right','fire']) || !is_int($run['ticks'] ?? null) || $run['ticks']<1 || $run['ticks']>50 || !is_bool($run['left'] ?? null) || !is_bool($run['right'] ?? null) || !is_bool($run['fire'] ?? null)) throw new MultiplayerGameException('Invalid input frames.','ARCADE_INPUT_INVALID',422);
            $ticks+=$run['ticks'];
        }
        if($ticks>50) throw new MultiplayerGameException('Too many input frames.','ARCADE_INPUT_INVALID',422);
