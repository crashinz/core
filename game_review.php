...
$pdo=db();$catalog=game_review_catalog();
$groups=[];foreach($catalog as $id=>$entry){$name=explode(' — ',$entry['label'])[0];$groups[$name][$id]=$entry;}uksort($groups,'strnatcasecmp');$caseId=(string)($_GET['example']??array_key_first($catalog));
...
try {
    ...
    if($_SERVER['REQUEST_METHOD']==='POST'){
        ...
        if($op==='start'){
            ...
            if($op==='reference-reset'){$review['referenceVersion']=(int)($review['referenceVersion']??((int)($review['referenceCursor']??0)+1))+1;$review['referenceCursor']=0;$review['referenceRequests']=[];}
            else {
                $requested=(int)($_POST['step']??-1);$cursor=(int)($review['referenceCursor']??0);
                if($requested<0||$requested>$cursor)throw new MultiplayerGameException('Refresh the reference controls.','GAME_REVIEW_STEP_CONFLICT',409);
                // A renderer may settle the same prepared trick/dealer action
                // before the button response arrives. Acknowledge, never skip
                // another action and never report a completed replay as broken.
                if($requested===$cursor)game_review_snapshot_step($review);
            }
...
$frozen=null;$frozenError='';if($review){try{
    ...
    if($baseStatus['missing'])throw new RuntimeException('Reference pack v5 is not fully installed. Open Reference pack to install its ZIP.');
...
<?php if(!empty($packDownload['url'])): ?><p><a href="<?=e($packDownload['url'])?>">Download reference pack <?=e($packDownload['version'])?> ZIP (<?=e((string)round($packDownload['bytes']/1000000))?> MB)</a> · <a href="<?=e($packDownload['releaseUrl'])?>" target="_blank" rel="noopener">Release details and checksum</a></p><?php else: ?><p>Install the supplied <?=e($packDownload['filename'])?> ZIP. This version has not been published for download yet.</p><?php endif; ?>
...
<form method="get" class="selector" id="review-selector"><label>Game<select id="review-game-filter"><?php foreach($groups as $name=>$items):?><option value="<?=e($name)?>" <?=isset($items[$caseId])?'selected':''?>><?=e($name)?></option><?php endforeach?></select></label><label>Example<select name="example" id="review-example"><?php foreach($groups as $name=>$items):?><optgroup label="<?=e($name)?>"><?php foreach($items as $id=>$item):?><option value="<?=e($id)?>" <?=$id===$caseId?'selected':''?>><?=e($item['label'])?></option><?php endforeach?></optgroup><?php endforeach?></select></label><label>Appearance<select name="pack"><?php foreach($packs as $id=>$item):?><option value="<?=e($id)?>" <?=$id===$pack?'selected':''?>><?=e($item['label']??ucfirst($id))?></option><?php endforeach?></select></label></form>
...
<?php if($review&&$review['steps']):?><form method="post" id="review-step" class="review-action" data-auto-opponent="<?=$caseId==='dominos-loss'?'true':'false'?>"><?=csrf_input()?><input type="hidden" name="operation" value="step"><input type="hidden" name="review_id" value="<?=e($review['id'])?>"><input type="hidden" name="step" value="<?=$review['step']?>"><button <?=$review['step']>=count($review['steps'])?'disabled':''?>><?=$caseId==='dominos-loss'?'Play opponent winning tile':'Play example action'?></button></form><?php endif?><span id="review-status" role="status" aria-live="polite"></span></div><?php if($url):?><iframe id="review-game" title="<?=e($case['label'])?> — Live example" src="<?=e($url)?>"></iframe><?php else:?><p>Start the example to load the live game.</p><?php endif?></section>
<section><h3>Frozen reference</h3>
<?php if($frozen):$frozenUrl='game_review_reference.php/v5/'.$frozen['entry'].'?'.http_build_query(['game_session_id'=>$review['id'],'participant_id'=>1,'user'=>1,'csrf'=>csrf_token()]);?>
...
<p class="minor">Use <strong>Play reference action</strong> above to advance each recorded move. Keyboard controls do not play a frozen arcade recording. Some examples demonstrate one move rather than a whole match. Use each game’s Sound FX control to hear one side at a time.</p>
...
</main><script src="assets/js/game-review.js?v=20260924-opponent"></script><script src="assets/js/game-review-pack.js"></script></body></html>
