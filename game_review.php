...
$frozen=null;$frozenError='';if($review){try{
    ...
    if($baseStatus['missing'])throw new RuntimeException('Reference pack v3 is not fully installed. Open Reference pack to download and install it.');
...
<?php if($frozen):$frozenUrl='game_review_reference.php/v3/'.$frozen['entry'].'?'.http_build_query(['game_session_id'=>$review['id'],'participant_id'=>1,'user'=>1,'csrf'=>csrf_token()]);?>
