...
$frozen=null;$frozenError='';if($review){try{
    ...
    if($baseStatus['missing'])throw new RuntimeException('Reference pack v2 is not fully installed. Open Reference pack to download and install it.');
...
<div class="comparison"><section><h3>Live example</h3><div class="toolbar pane-controls"><form method="post"><?=csrf_input()?><input type="hidden" name="operation" value="start"><input type="hidden" name="review_id" value="<?=e($review['id']??'')?>"><button class="primary"><?=$review?'Reset and repeat':'Start example'?></button></form>
...
<?php if($frozen):$frozenUrl='game_review_reference.php/v2/'.$frozen['entry'].'?'.http_build_query(['game_session_id'=>$review['id'],'participant_id'=>1,'user'=>1,'csrf'=>csrf_token()]);?>
...
<iframe id="review-reference" title="<?=e($case['label'])?> — Frozen reference" src="<?=e($frozenUrl)?>"></iframe><p class="minor">Saved <?=e(game_review_snapshot_manifest()['createdAt'])?> · <?=e(substr(game_review_snapshot_manifest()['source']['release'],0,16))?>. This renderer, artwork and prepared sequence stay fixed when the live game changes.</p>
