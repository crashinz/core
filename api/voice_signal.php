...
if ($_SERVER['REQUEST_METHOD'] === 'GET') {
    ...
    $stmt = $pdo->prepare('SELECT v.participant_id, p.display_name FROM voice_sessions v JOIN participants p ON p.id = v.participant_id WHERE v.session_id = ?');
    $stmt->execute([$sessionId]);
    $voice = array_map(fn($r) => ['id' => (int)$r['participant_id'], 'display_name' => $r['display_name']], $stmt->fetchAll());
    ...
    if ($action === 'join' || $action === 'leave') {
        ...
        if ($action === 'join') {
            $pdo->prepare(db_uses_mysql_syntax($pdo)
                ? 'INSERT INTO voice_sessions (participant_id, session_id, joined_at) VALUES (?,?,CURRENT_TIMESTAMP) ON DUPLICATE KEY UPDATE session_id = VALUES(session_id), joined_at = CURRENT_TIMESTAMP'
                : 'INSERT OR REPLACE INTO voice_sessions (participant_id, session_id, joined_at) VALUES (?,?,CURRENT_TIMESTAMP)'
