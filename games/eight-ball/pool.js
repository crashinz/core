import {createTurnClock} from './turn-clock.js?v=d6538f01b4b6';
...
const canAct=()=>!turnClock.expired()&&network&&network.status==='active'&&!network.busy&&!sending&&!playback&&Number(network.state.turnOrder?.[network.state.turnIndex])===network.currentUserId&&!network.state.completed;
...
function loop(now){turnClock.tick();const dt=Math.min(.035,(now-last)/1000||.016);last=now;if(!activePanel&&!moving&&!placement&&document.activeElement===canvas){const fine=keys.has('Shift')?.25:1;if(now-aimKeyStarted>=300){if(keys.has('ArrowLeft'))adjust('aim-left',dt*88*fine);if(keys.has('ArrowRight'))adjust('aim-right',dt*88*fine);}if(keys.has('ArrowUp'))adjust('power-up',dt*28);if(keys.has('ArrowDown'))adjust('power-down',dt*28);if(keys.size)updateUI();}if(hold&&!moving&&now-hold.start>300){adjust(hold.action,dt*(hold.action.startsWith('aim-')?88:22));}if(moving){accumulator+=dt;while(accumulator>=1/240&&moving){step(1/240);accumulator-=1/240;}}else window.PoolPhysics.idle(balls,dt);updateCursor();draw();requestAnimationFrame(loop);}
...
document.addEventListener('wheel',e=>{
 if(e.ctrlKey||activePanel?.contains(e.target)||e.target.closest?.('.match-actions,.table-choice')||parent===window)return;
...
function resizeCanvas(){const ratio=Math.min(window.devicePixelRatio||1,2),w=Math.round(Math.max(600,canvas.clientWidth)*ratio),h=Math.round(w*viewH/viewW);if(canvas.width!==w||canvas.height!==h){canvas.width=w;canvas.height=h;}ctx.setTransform(w/viewW,0,0,h/viewH,cueSpace.left*w/viewW,cueSpace.top*h/viewH);
 const panel=$('#tableChoice'),scale=canvas.clientWidth/viewW;panel.style.setProperty('--choice-left',100*(cueSpace.left+(box.l+box.r)/2)/viewW+'%');panel.style.setProperty('--choice-top',100*(cueSpace.top+(box.t+box.b)/2)/viewH+'%');panel.style.setProperty('--choice-width',Math.max(140,(box.r-box.l)*scale-24)+'px');panel.style.setProperty('--choice-height',Math.max(90,(box.b-box.t)*scale-24)+'px');draw();}
...
function renderTableChoice(st){
 const panel=$('#tableChoice'),actions=$('#tableChoiceActions'),wasOpen=!panel.hidden,hadFocus=panel.contains(document.activeElement);
 const show=network.status==='active'&&!st.completed&&!playback&&st.phase==='choice'&&st.turnOrder?.[st.turnIndex]===network.currentUserId;
 panel.hidden=!show;actions.replaceChildren();
 if(!show){if(hadFocus)canvas.focus({preventScroll:true});return;}
 const illegal=st.choice?.startsWith('illegal'),scratch=st.choice?.endsWith('scratch'),pushOut=st.choice==='push-out';
 $('#tableChoiceTitle').textContent=pushOut?'Choose who shoots':illegal?'Choose how to continue':scratch?'8 ball and cue ball pocketed':'8 ball pocketed on the break';
 $('#tableChoiceText').textContent=pushOut?'Your opponent played a push out. Take this shot or let your opponent shoot again.':illegal?'Keep this table, or rerack all the balls and choose who takes the first shot.':scratch?'Put the 8 ball back and place the cue ball behind the starting line, or rerack all the balls and take the first shot.':'Put the 8 ball back and continue, or rerack all the balls and take the first shot.';
 const choices=pushOut?[['Take shot','take'],['Return shot','return']]:[[illegal?'Keep this table':'Put the 8 ball back',illegal?'accept':'spot'],['Rerack','rerack-self'],...(illegal?[['Rerack for opponent','rerack-other']]:[])];
 for(const [label,choice]of choices){const b=document.createElement('button');b.type='button';b.textContent=label;b.disabled=!canAct();b.onclick=()=>{for(const button of actions.children)button.disabled=true;postAction('choice',{choice});};actions.append(b);}
 if(!wasOpen||hadFocus)actions.querySelector('button:not(:disabled)')?.focus({preventScroll:true});
}
...
function receiveNetwork(data){const previous=network;if(playback?.replay&&(data.sessionId!==previous?.sessionId||data.state?.sequence!==previous?.state?.sequence||data.status!=='active'||data.state?.completed))endReplay();network=data;turnClock.observe(data);sending=false;const st=data.state||{};R=st.ballRadius===15.5?15.5:12.5;if(previous?.sessionId!==data.sessionId||!st.schemaVersion||(!st.lastShot&&previous?.state?.lastShot)){playback=null;pendingShots=[];playedShot='';networkVersion='';turnGuide={key:'',startedAt:0,ids:[]};rollingBallArt.reset();}if(!st.schemaVersion){$('#tableChoice').hidden=true;moving=false;balls=[];status('Start the game in the waiting lobby.');draw();return;}
