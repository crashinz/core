...
export function createChineseCheckersBotController({snapshot,submit,showStatus,WorkerClass=globalThis.Worker}){
  ...
  function sync(){
    ...
    try{
      const worker=new WorkerClass(new URL('./chinese-checkers-worker.js?v=0600d320ec1a',import.meta.url),{type:'module'});active.worker=worker;
      active.timeout=setTimeout(()=>fail(active,'The Chinese Checkers bot took too long. Retry this move.'),Math.max(5000,task.moveTimeMs+3000));
      worker.onerror=e=>{e?.preventDefault?.();fail(active,'The Chinese Checkers bot could not load. Check your connection, then retry.');};
      worker.onmessage=({data})=>{
        ...
        if(data.type!=='result'||!/^r\d{2}c\d{2}$/.test(data.from)||!/^r\d{2}c\d{2}$/.test(data.to)){
        ...
        active.delay=setTimeout(async()=>{
          ...
          try{
            const ok=await submit({engine:task.engine,positionKey:task.positionKey,from:data.from,to:data.to,elapsedMs:data.elapsedMs});
    ...
    }catch{fail(active,'The Chinese Checkers bot could not start. Retry in a current browser.');}
  }
  return {sync,stop};
}
